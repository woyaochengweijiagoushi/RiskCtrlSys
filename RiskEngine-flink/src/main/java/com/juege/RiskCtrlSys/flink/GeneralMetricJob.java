package com.juege.RiskCtrlSys.flink;
...
import com.juege.RiskCtrlSys.flink.job.window.MetricTrigger;
 ...
 * 4. 通用聚合计算框架只是参数的不同, 运营人员可以在后台配置这些参数,
 *    例如按照哪些字段分组，按照什么时间字段进行窗口计算等等，
 *    从而可以根据后台配置的参数自动的生成指标计算Job
 *
 *
 * 指标的通用聚合计算框架的设计思路
 * 1. 通过运营后台配置指标的聚合计算的规则,
 * 2. 每个Job只需要调用通用的模块, 这些通用模块去读取运营后台对应指标的配置和计算规则, 然后进行聚合计算
 *
 *
        ...
        KafkaUtil
                ...
                .assignTimestampsAndWatermarks(
                        WatermarkStrategy
                                ...
                                .<EventPO>forBoundedOutOfOrderness(Duration.ofMillis(tool.getInt(ParameterConstantsUtil.FLINK_MAXOUTOFORDERNESS) * 10000L))
